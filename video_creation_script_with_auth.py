...
def generate_image(prompt, retries=5, initial_wait=10):
    ...
    for idx, description in enumerate(scene_descriptions):
        ...
        if description.strip():
            ...
            try:
                ...
                # Add a delay between image generations to respect rate limits
                if idx < total_scenes - 1:  # Don't wait after the last image
                    wait_time = 12  # Wait for 12 seconds between image generations
                    print(f"Waiting for {wait_time} seconds before generating the next image...")
                    time.sleep(wait_time)
